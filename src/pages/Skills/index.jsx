import { Col, Container, Row } from "react-bootstrap";
import Carousel from "react-multi-carousel";
import "react-multi-carousel/lib/styles.css";
import meter1 from "../../assets/imgs/meter1.svg";
import meter2 from "../../assets/imgs/meter2.svg";
import meter3 from "../../assets/imgs/meter3.svg";
import colorSharp from "../../assets/imgs/color-sharp.png";

const Skills = () => {
  const responsive = {
    superLargeDesktop: {
      // the naming can be any, depends on you.
      breakpoint: { max: 4000, min: 3000 },
      items: 5,
    },
    desktop: {
      breakpoint: { max: 3000, min: 1024 },
      items: 3,
    },
    tablet: {
      breakpoint: { max: 1024, min: 464 },
      items: 2,
    },
    mobile: {
      breakpoint: { max: 464, min: 0 },
      items: 1,
    },
  };

  return (
    <section className="skill" id="skills">
      <Container>
        <Row>
          <Col>
            <div className="skill-bx">
              <h2>Skills</h2>
              <p>My skills and states!</p>
              <Carousel
                responsive={responsive}
                infinite={"true"}
                className="skill-slider"
              >
                <div className="item">
                  <img src={meter3} alt="skill-icon" />
                  <h5>JavaScript</h5>
                </div>
                <div className="item">
                  <img src={meter1} alt="skill-icon" />
                  <h5>Web development</h5>
                </div>
                <div className="item">
                  <img src={meter3} alt="skill-icon" />
                  <h5>Cross platform mobile apps development</h5>
                </div>
                <div className="item">
                  <img src={meter2} alt="skill-icon" />
                  <h5>Node Js, Express & MongoDB</h5>
                </div>
                <div className="item">
                  <img src={meter1} alt="skill-icon" />
                  <h5>State mangament</h5>
                </div>
                <div className="item">
                  <img src={meter1} alt="skill-icon" />
                  <h5>Concepts(OOP, Design Patterns & Solid principles)</h5>
                </div>
                <div className="item">
                  <img src={meter3} alt="skill-icon" />
                  <h5>Algorithms and Data structure</h5>
                </div>
              </Carousel>
            </div>
          </Col>
        </Row>
      </Container>
      <img src={colorSharp} alt="" className=" background-image-left" />
    </section>
  );
};

export default Skills;
